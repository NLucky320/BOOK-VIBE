...
const BookDetails = () => {
  ...
  return (
    <Card className="w-full flex-col items-center md:flex-row my-[50px] gap-4 p-4  mt-[80px]">
      <CardHeader
        shadow={false}
        floated={false}
      ...
      <CardBody className="space-y-2 p-4">
        ...
        <Typography
          color="gray"
          className="font-normal border-b border-dashed p-2 w-full"
        >
          By: {author}
        </Typography>
        <Typography
          color="gray"
          className="font-normal border-b border-dashed p-2"
        >
          {category}
        </Typography>
        ...
        <div className="font-normal border-b border-dashed p-2">
          <div className="flex gap-2 items-center">
            ...
            {tags &&
              tags.map((tag, index) => (
                <p
                  className="text-green-500 bg-green-100 rounded-full p-2 px-6"
                  key={index}
                >
                  #{tag}
                </p>
        ...
        <div className="items-center flex-shrink flex gap-4">
          <button
            ...
            className="self-center px-8 py-3 text-[#131313] rounded border border-solid border-[#1313134D]"
          >
            Read
          </button>
          <button
            onClick={handleSaveToWishlist}
            className="self-center px-8 py-3 font-semibold rounded bg-[#59C6D2] text-white"
          >
            Wishlist
          </button>
        </div>
